...
parser = argparse.ArgumentParser(description='DDLMVS for high-resolution multi-view stereo')
parser.add_argument('--mode', default='train', help='train or val', choices=['train', 'val'])
parser.add_argument('--model', default='DDLMVS', help='select model')
...
parser.add_argument('--lrepochs', type=str, default="10,12,14,18,25,30,35,40:2", help='epoch ids to downscale lr and the downscale rate')
...
# model, optimizer
model = DDLMVS(patchmatch_interval_scale=args.patchmatch_interval_scale,
...
def train_sample(sample, detailed_summary=False):
    ...
    edge_est = outputs["refined_edge"]
    depth_patchmatch = outputs["depth_patchmatch"]
    pseudo_pi = outputs["pseudo_pi"]
    pi = outputs["pi"]
    mu0 = outputs["mu0"]
    mu1 = outputs["mu1"]
    sigma0 = outputs["sigma0"]
    sigma1 = outputs["sigma1"]

    loss = model_loss(depth_patchmatch, depth_est,edge_est, depth_gt, mask,pi, mu0,  mu1, sigma0, sigma1)
    loss.backward()
    optimizer.step()
